...
func callFunction(clientObj *Client, requestObj *requestObject) {
	...
	// 定义调用方法的时间
	timeOfBeforeCallFunc := time.Now().Unix()
	timeOfAfterCallFunc := time.Now().Unix()

	defer func() {
		// 如果调用方法的时间超过1s则记录下来(如果是Fight，则需要超过5s才记录)
		callSeconds := timeOfAfterCallFunc - timeOfBeforeCallFunc
		if (requestObj.MethodName != "Fight" && callSeconds > 1) || (requestObj.MethodName == "Fight" && callSeconds > 5) {
			redisMgr.RecordExpireLog(fmt.Sprintf("Module:%s,Method:%s;CallFuncTime:%d;", requestObj.ModuleName, requestObj.MethodName, callSeconds))
		}
	}()
	...
	if methodAndInOutTypes, ok = methodMap[key]; !ok {
		clientObj.WriteLog(fmt.Sprintf("找不到指定的方法：%s", key))
		ResponseResult(clientObj, requestObj, responseObj.SetResultStatus(MethodNotDefined), Con_HighPriority)
		return
	}
	...
	if paramLength != inTypesLength {
		clientObj.WriteLog(fmt.Sprintf("传入的参数数量不符，本地方法%s的参数数量：%d，传入的参数数量为：%d", key, inTypesLength, paramLength))
		ResponseResult(clientObj, requestObj, responseObj.SetResultStatus(ParamNotMatch), Con_HighPriority)
		return
	}
	...
	for _, item := range in {
		if reflect.Value.IsValid(item) == false {
			clientObj.WriteLog(fmt.Sprintf("type:%v,value:%v.方法%s传入的参数%v无效", reflect.TypeOf(item), reflect.ValueOf(item), key, requestObj.Parameters))
			ResponseResult(clientObj, requestObj, responseObj.SetResultStatus(ParamInValid), Con_HighPriority)
			return
		}
	}
	...
	if requestObj.MethodName != "Beat" {
		clientObj.setCurFuncName(fmt.Sprintf("模块：%v,方法:%v", requestObj.ModuleName, requestObj.MethodName))
	}

	// 并输出结果到客户端（由于只有一个返回值，所以取out[0]）
	ResponseResult(clientObj, requestObj, (&out[0]).Interface(), Con_HighPriority)
}
